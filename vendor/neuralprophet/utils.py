...
from vendor.neuralprophet import utils_torch
from vendor.neuralprophet.hdays_utils import get_country_holidays
from vendor.neuralprophet.logger import ProgressBar

if TYPE_CHECKING:
    from vendor.neuralprophet.configure import ConfigEvents, ConfigLaggedRegressors, ConfigSeasonality, Train
...
def save(forecaster, path):
    ...
    After you fitted a model, you may save the model to save_test_model.np
        >>> from vendor.neuralprophet import save
...
def load(path):
    ...
    Saved model could be loaded from disk file test_save_model.np
        >>> from vendor.neuralprophet import load
