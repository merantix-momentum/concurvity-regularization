...
from vendor.neuralprophet import configure, df_utils, np_types, time_dataset, time_net, utils, utils_metrics
from vendor.neuralprophet.conformal import Conformal
from vendor.neuralprophet.logger import MetricsLogger
from vendor.neuralprophet.plot_forecast_matplotlib import plot, plot_components
from vendor.neuralprophet.plot_forecast_plotly import plot as plot_plotly
from vendor.neuralprophet.plot_forecast_plotly import plot_components as plot_components_plotly
from vendor.neuralprophet.plot_model_parameters_matplotlib import plot_parameters
from vendor.neuralprophet.plot_model_parameters_plotly import plot_parameters as plot_parameters_plotly
from vendor.neuralprophet.plot_utils import get_valid_configuration, log_warning_deprecation_plotly
...
class NeuralProphet:
    ...
    def __init__(
        ...
        self.metrics_logger = MetricsLogger(save_dir=os.getcwd()) if not wandb.run else WandbLogger(log_model="all")
    ...
    def fit(
        ...
        early_stopping_target: str = "MAE_val",
    ...
    def _train(
        ...
        metrics_df = (
            pd.DataFrame(self.metrics_logger.history)
            if not wandb.run
            else {k: v.item() for k, v in self.trainer.logged_metrics.items()}
        )

        if hasattr(checkpoint_callback, "best_model_score"):
            metrics_df[f"{checkpoint_callback.monitor}(best)"] = checkpoint_callback.best_model_score
            if wandb.run:
                wandb.log({f"{checkpoint_callback.monitor}(best)": checkpoint_callback.best_model_score})
        return metrics_df
